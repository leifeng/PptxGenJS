...
function slideObjectToXml(slide: PresSlide | SlideLayout): string {
    ...
    switch (slideItemObj._type) {
      ...
      case SLIDE_OBJECT_TYPES.placeholder:
        ...
        if (slideItemObj.options.gradient) {
          ...
          if (slideItemObj.options.gradient.type === 'linear') {
            ...
            strSlideXml +=
              '<a:path path="circle"><a:fillToRect l="50000" t="50000" r="50000" b="50000" /></a:path><a:tileRect />';
          }
      ...
      case SLIDE_OBJECT_TYPES.image:
        ...
        if (slideItemObj.options.radius) {
          // 圆角
          strSlideXml +=
            ' <a:prstGeom prst="roundRect"><a:avLst><a:gd name="adj" fmla="val 8594"/></a:avLst></a:prstGeom>';
        } else {
  ...
  if (slide._slideNumberProps) {
    ...
    if (
      ...
      slide._slideNumberProps.fontFaceEn ||
...
function genXmlTextRunProperties(
  ...
  runProps +=
    ...
    runPropsTag +
    ' lang="' +
    (opts.lang ? opts.lang : 'zh-CN') +
    '"' +
    (opts.lang ? ' altLang="en-US"' : ' altLang="en-US"');
  ...
  arrLines.forEach((line) => {
    ...
    line.forEach((textObj, idx) => {
      ...
      if (
        ...
        textObj.text.includes('<m:oMath') &&
        textObj.options.isMath
      ) {
    ...
    if (slideObj._type === SLIDE_OBJECT_TYPES.tablecell && (opts.fontSize || opts.fontFace)) {
      if (opts.fontFace) {
        strSlideXml +=
          `<a:endParaRPr lang="${opts.lang || 'zh-CN'}" altLang="en-US"` +
